document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderQuestions() {
    ...
    shownQuestions.forEach((q, idx) => {
      ...
      btnTranslate.onclick = () => speakText(q.translate, "es-ES");
  ...
  function showPronounceModal(original, spoken) {
    ...
    for (let i = 0; i < s.length; i++) {
      ...
      if (matchAtPos) spokenHtml += `<span class="correct-word">${w}</span> `;
      else spokenHtml += `<span class="incorrect-word">${w}</span> `;
    }
    spokenHtml += `</p>`;

    let originalHtml = `<p><strong>Target:</strong> ` + o.join(" ") + `</p>`;
  ...
  function grade() {
    ...
    cards.forEach(card => {
      ...
      const selected = inputs.filter(i => i.checked).map(i => i.value.trim());
      const correctAnswers = (Array.isArray(q.correct) ? q.correct : [q.correct]).map(x => x.trim());
      if (JSON.stringify(selected.sort()) === JSON.stringify(correctAnswers.sort())) correctCount++;

      inputs.forEach(inp => {
        const label = inp.closest(".option-label");
        const val = inp.value.trim();
        if (correctAnswers.includes(val) && inp.checked) label.classList.add("correct");
        else if (correctAnswers.includes(val) && !inp.checked) label.classList.add("missed");
        else if (!correctAnswers.includes(val) && inp.checked) label.classList.add("incorrect");
      });
    });

    btnInfo.textContent = `Info (${correctCount} correct / ${cards.length - correctCount} incorrect)`;
  }
  ...
  function reviewIncorrect() {
    ...
    w.document.write(`<html><head><title>Review Incorrect</title>
      <style>body{font-family:Arial,sans-serif;padding:20px;background:#f0f2f5;} .q{background:white;padding:15px;border-radius:8px;margin-bottom:12px;box-shadow:0 2px 6px rgba(0,0,0,0.08);} .title{font-weight:bold;color:#d63031;} .correct{color:#00a86b;font-weight:bold;}</style>
    ...
    cards.forEach((card, idx) => {
      ...
      if (JSON.stringify(selected.sort()) !== JSON.stringify(correctAnswers.sort())) {
        w.document.body.innerHTML += `<div class="q">
